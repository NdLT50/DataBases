...
SELECT project_name, budget, CASE WHEN budget > 150000 THEN 'Large' WHEN budget BETWEEN 100000 AND 150000 THEN 'Medium' ELSE 'Small' END AS budget_category FROM projects;
...
SELECT UPPER(CONCAT(first_name, ' ', last_name)) AS upper_name, LENGTH(last_name) AS last_name_length, SUBSTRING(email FROM 1 FOR 3) AS email_prefix FROM employees;

SELECT first_name, last_name, salary * 12 AS annual_salary, ROUND(salary / 12, 2) AS monthly_salary, salary * 0.10 AS raise_amount FROM employees;

SELECT FORMAT('Project: %s - Budget: $%s - Status: %s', project_name, budget, status) AS project_info FROM projects;

SELECT first_name, last_name, DATE_PART('year', AGE(CURRENT_DATE, hire_date)) AS years_with_company FROM employees;

SELECT department, AVG(salary) AS avg_salary FROM employees GROUP BY department;

SELECT p.project_name, SUM(a.hours_worked) AS total_hours FROM assignments a JOIN projects p ON a.project_id = p.project_id GROUP BY p.project_name;
...
WHERE salary > 65000 UNION SELECT employee_id, CONCAT(first_name, ' ', last_name), salary FROM employees WHERE hire_date > '2020-01-01';

SELECT * FROM employees WHERE department = 'IT' INTERSECT SELECT * FROM employees WHERE salary > 65000;

SELECT * FROM employees EXCEPT SELECT e.* FROM employees e JOIN assignments a ON e.employee_id = a.employee_id;

SELECT * FROM employees e WHERE EXISTS (SELECT 1 FROM assignments a WHERE a.employee_id = e.employee_id);

SELECT * FROM employees WHERE employee_id IN (SELECT a.employee_id FROM assignments a JOIN projects p ON a.project_id = p.project_id WHERE p.status = 'Active');
...
FROM projects p JOIN assignments a ON p.project_id = a.project_id GROUP BY p.project_name HAVING SUM(a.hours_worked) > 150;
...
SELECT 
    ...
    GREATEST(AVG(salary), MAX(salary)) AS salary_check,
    LEAST(MIN(salary), AVG(salary)) AS low_salary_check
FROM employees e1
GROUP BY department;
