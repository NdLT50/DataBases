...
-- Indexes = 2 (PRIMARY KEY + emp_salary_idx)

-- 2.2
CREATE INDEX emp_dept_idx ON employees(dept_id);

SELECT * FROM employees WHERE dept_id = 101;
-- FK columns often used in JOIN/WHERE → indexing speeds up queries
...
-- Automatically created indexes = primary keys
...
-- Yes, order matters. Index works best by the first column.
...
-- PostgreSQL creates B-TREE index automatically.
...
-- Index avoids sorting, speeds up ORDER BY DESC
...
-- Without this, PostgreSQL must scan all rows (Seq Scan)
...
-- Drop index if unused, duplicate, or slows writes

-- 7.3
REINDEX INDEX employees_salary_index;
-- REINDEX fixes bloat after many INSERT/UPDATE/DELETE
...
FROM projects
WHERE budget > 80000;
-- Partial index = smaller, faster, only for needed rows

-- 8.3
EXPLAIN SELECT * FROM employees WHERE salary > 52000;
-- If Index Scan → index used. Seq Scan → index NOT used.
...
-- Range queries work only on B-tree
...
WHERE schemaname='public'
ORDER BY tablename, indexname;
-- Largest index = on columns with biggest data or long strings
...
-- 2.Use index for WHERE, JOIN, ORDER BY
-- 3.Do NOT index small tables or rarely-used columns
-- 4.INSERT/UPDATE/DELETE slow down because index updates
